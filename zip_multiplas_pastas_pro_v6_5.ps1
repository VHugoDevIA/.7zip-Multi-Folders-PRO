...
$AppTitle = "ZIP Multiplas Pasta - PRO v6.5"
$MailSubject = "ZIP Multiplas Pasta - PRO v6.5"
$StateFile = Join-Path -Path $env:APPDATA -ChildPath "VHugoDevIA\zip_multiplas_pastas_pro_v6_5_state.json"
        ...
        try {
            ...
            elseif ($exitCode -eq 0) {
                ...
                if ($chkDelete.Checked) {
                    $removedOk = Remove-OriginalFolderAfterSuccess -FolderPath $folderPath
                    if ($removedOk) {
                        Add-LogLine -TextBox $txtLog -Text "[OK] Originais eliminados após compressão: $folderName"
                    }
                    else {
                        Add-LogLine -TextBox $txtLog -Text "[AVISO] Não foi possível eliminar a pasta original: $folderPath"
                    }
                }
            }
            else {
